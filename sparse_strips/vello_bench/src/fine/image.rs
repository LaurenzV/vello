...
mod extend {
    ...
    fn extend_base<F: Type>(b: &mut Bencher<'_>, fine: &mut Fine<F>, extend: peniko::Extend) {
    ...
    #[vello_bench]
    fn pad<F: Type>(b: &mut Bencher<'_>, fine: &mut Fine<F>) {
        extend_base(b, fine, peniko::Extend::Pad);
    }

    #[vello_bench]
    fn repeat<F: Type>(b: &mut Bencher<'_>, fine: &mut Fine<F>) {
        extend_base(b, fine, peniko::Extend::Repeat);
    }

    #[vello_bench]
    fn reflect<F: Type>(b: &mut Bencher<'_>, fine: &mut Fine<F>) {
...
mod quality {
    ...
    fn quality_base<F: Type>(b: &mut Bencher<'_>, fine: &mut Fine<F>, quality: ImageQuality) {
        let im = get_colr_image(peniko::Extend::Pad, quality);
        image_base(b, fine, im, Affine::scale(3.0))
    }

    #[vello_bench]
    fn low<F: Type>(b: &mut Bencher<'_>, fine: &mut Fine<F>) {
        quality_base(b, fine, ImageQuality::Low);
    }

    #[vello_bench]
    fn medium<F: Type>(b: &mut Bencher<'_>, fine: &mut Fine<F>) {
        quality_base(b, fine, ImageQuality::Medium);
    }

    #[vello_bench]
    fn high<F: Type>(b: &mut Bencher<'_>, fine: &mut Fine<F>) {
...
mod transform {
    ...
    #[vello_bench]
    fn none<F: Type>(b: &mut Bencher<'_>, fine: &mut Fine<F>) {
        let im = get_colr_image(peniko::Extend::Pad, ImageQuality::Low);
        image_base(b, fine, im, Affine::IDENTITY)
    }

    #[vello_bench]
    fn scale<F: Type>(b: &mut Bencher<'_>, fine: &mut Fine<F>) {
        let im = get_colr_image(peniko::Extend::Pad, ImageQuality::Low);
        image_base(b, fine, im, Affine::scale(3.0))
    }

    #[vello_bench]
    fn rotate<F: Type>(b: &mut Bencher<'_>, fine: &mut Fine<F>) {
        let im = get_colr_image(peniko::Extend::Pad, ImageQuality::Low);
        image_base(
            b,
            fine,
...
fn image_base<F: Type>(b: &mut Bencher<'_>, fine: &mut Fine<F>, image: Image, transform: Affine) {
    let mut paints = vec![];
