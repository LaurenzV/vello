...
impl Type for f32x8 {
    ...
    fn load(src: &[f32]) -> Self {
        Self(f32x4::load(&src[0..4]), f32x4::load(&src[4..8]))
    }

    #[inline(always)]
    fn splat_4(src: [f32; 4]) -> Self {
        Self(f32x4::splat_4(src), f32x4::splat_4(src))
    }

    #[inline(always)]
    fn splat(value: f32) -> Self {
        Self(f32x4::splat(value), f32x4::splat(value))
    }

    #[inline(always)]
    fn store(self, dest: &mut [f32]) {
        self.0.store(&mut dest[0..4]);
        self.1.store(&mut dest[4..8]);
    }
